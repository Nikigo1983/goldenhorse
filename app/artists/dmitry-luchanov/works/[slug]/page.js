...
import { luchanovWorksData } from "../luchanovWorksData";

const worksBasePath = "/artists/dmitry-luchanov/works";

export default function DmitryLuchanovWorkDetailPage() {
  const router = useRouter();
  const params = useParams();
  const slug = typeof params?.slug === "string" ? params.slug : "";
  const work = useMemo(() => luchanovWorksData.find((item) => item.slug === slug) ?? null, [slug]);
  const workIndex = useMemo(() => luchanovWorksData.findIndex((item) => item.slug === slug), [slug]);
  ...
  const goToWorkByOffset = (offset) => {
    const total = luchanovWorksData.length;
    const nextIndex = (workIndex + offset + total) % total;
    const nextWork = luchanovWorksData[nextIndex];
    if (nextWork) {
      ...
      <section className="section page-section artist-work-detail-page">
        ...
        <div className="artist-work-detail-grid">
          ...
          <div className="artist-work-detail-image-wrap">
            ...
            {work.sold ? <span className="artist-work-sold-badge artist-work-sold-badge-detail">Sold</span> : null}
            <button
              type="button"
          ...
          <div className="enquiry-modal" role="dialog" aria-modal="true" aria-label="Enquiry form" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="enquiry-modal-scroll">
              ...
              <div className="enquiry-footer-note">
                ...
                <p>
                  In order to respond to your enquiry, we will process the personal data you have supplied to communicate
                  with you in accordance with our <Link href="/privacy">Privacy Policy</Link>.
                </p>

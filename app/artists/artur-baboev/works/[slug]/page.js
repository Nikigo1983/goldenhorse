...
export default function ArturBaboevWorkDetailPage() {
  const router = useRouter();
  const pathname = usePathname();
  const params = useParams();
  const slug = typeof params?.slug === "string" ? params.slug : "";
  const work = useMemo(() => worksData.find((item) => item.slug === slug) ?? null, [slug]);
  const workIndex = useMemo(() => worksData.findIndex((item) => item.slug === slug), [slug]);
  ...
  const goToWorkByOffset = (offset) => {
    ...
    const nextWork = worksData[nextIndex];
    if (nextWork) {

...
export default function PublishingWorksPage() {
  ...
  const selectedWork = useMemo(
    () => publishingWorksData.find((w) => w.id === selectedWorkId) ?? null,
    [selectedWorkId]
  );

  return (
    <>
      <section className="section page-section artist-works-page publishing-works-page">
        <h1 className="category-landing-title">PUBLISHING</h1>

        <div className="artist-works-grid publishing-works-grid">
          {publishingWorksData.map((work) => (
            <article key={work.id} className={`artist-works-card ${work.cardClass}`}>
              <Link href={`${worksBasePath}/${work.slug}`}>
                <img src={work.images[0]} alt={work.title} className="publishing-work-image" />
              </Link>
              <h3>{work.title}</h3>
              <button
                type="button"

...
import { porcelainWorksData } from "../porcelainWorksData";

const worksBasePath = "/artworks/porcelain";

export default function PorcelainWorkDetailPage() {
  const router = useRouter();
  const params = useParams();
  const slug = typeof params?.slug === "string" ? params.slug : "";
  const work = useMemo(() => porcelainWorksData.find((item) => item.slug === slug) ?? null, [slug]);
  const workIndex = useMemo(() => porcelainWorksData.findIndex((item) => item.slug === slug), [slug]);
  ...
  const [porcelainMainTitle, porcelainSubTitle] = work.title.split(",").map((part) => part.trim());
  ...
  const goToWorkByOffset = (offset) => {
    const total = porcelainWorksData.length;
    const nextIndex = (workIndex + offset + total) % total;
    const nextWork = porcelainWorksData[nextIndex];
    if (nextWork) {
      ...
      <section className="section page-section artist-work-detail-page">
        ...
        <div className="artist-work-detail-grid">
          <aside className="artist-work-detail-meta">
            <h1 className="porcelain-work-main-title">{porcelainMainTitle || work.title}</h1>
            {porcelainSubTitle ? <h2 className="porcelain-work-sub-title">{porcelainSubTitle}</h2> : null}
            ...
            {work.images.length > 1 ? (
              <div className="artist-work-further">
                ...
                <div className="artist-work-thumbs porcelain-work-thumbs">

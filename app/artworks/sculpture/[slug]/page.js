...
import { sculptureWorksData } from "../sculptureWorksData";

const worksBasePath = "/artworks/sculpture";

export default function SculptureWorkDetailPage() {
  const router = useRouter();
  const params = useParams();
  const slug = typeof params?.slug === "string" ? params.slug : "";
  const work = useMemo(() => sculptureWorksData.find((item) => item.slug === slug) ?? null, [slug]);
  const workIndex = useMemo(() => sculptureWorksData.findIndex((item) => item.slug === slug), [slug]);
  ...
  if (!work) {
    return (
      <section className="section page-section artist-work-detail-page sculpture-work-detail-page">
        <p>Work not found.</p>
      </section>
    );
  }
  ...
  const goToWorkByOffset = (offset) => {
    const total = sculptureWorksData.length;
    const nextIndex = (workIndex + offset + total) % total;
    const nextWork = sculptureWorksData[nextIndex];
    if (nextWork) {
      ...
      <section className="section page-section artist-work-detail-page">
        ...
        <div className="artist-work-detail-grid">
          <aside className="artist-work-detail-meta">
            <h2 className="sculpture-work-main-title">{work.title}</h2>

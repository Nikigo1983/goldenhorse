...
export default function SculptureWorksPage() {
  const [selectedWorkId, setSelectedWorkId] = useState(null);
  const selectedWork = useMemo(
    () => sculptureWorksData.find((w) => w.id === selectedWorkId) ?? null,
    [selectedWorkId]
  );

  return (
    <>
      <section className="section page-section artist-works-page sculpture-works-page">
        <h1 className="category-landing-title">SCULPTURE</h1>

        <div className="artist-works-grid">
          {sculptureWorksData.map((work) => (
            <article key={work.id} className={`artist-works-card ${work.cardClass}`}>
              <Link href={`${worksBasePath}/${work.slug}`}>
                <img src={work.images[0]} alt={work.title} className="sculpture-work-image" />
              </Link>
              <h3>{work.title}</h3>
              <button
                type="button"
